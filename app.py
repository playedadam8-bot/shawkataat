...
def generate_prop_vision_signal(api_key: str, image_bytes: bytes, account_balance: float, risk_pct: float):
    ...
    11. Setup Confidence Score on an integer scale from 0 to 10 (where 10 is an A+ pristine setup).

    CRITICAL INSTRUCTIONS:
    - Output ONLY valid raw JSON. Do NOT include markdown code blocks.
    - Provide exact keys: "asset", "live_price", "signal", "entry_price", "stop_loss_price", "take_profit_price", "risk_reward_ratio", "recommended_risk_amount", "suggested_lot_size", "accuracy", "setup_score", "reason", "estimated_entry_time", "estimated_tp_time".
...
# ------------------------------------------------------------------------------
# Sidebar Configuration (Persistent API Key Input)
# ------------------------------------------------------------------------------
st.sidebar.title("🛡️ Prop Control Panel")

# Persistent Secret Key Input bound to session state
st.session_state.api_key = st.sidebar.text_input(
    "Secret Key", 
    value=st.session_state.api_key, 
    type="password",
    help="Your OpenRouter secret key. Stored securely in your session state."
)
...
# ------------------------------------------------------------------------------
# Display Active Signal Output Area with Timing, Score & Thesis
# ------------------------------------------------------------------------------
if st.session_state.last_signal:
    ...
    with res_col1:
        ...
        st.write(f"**Risk-to-Reward (R:R):** `{sig_data.get('risk_rubber_ratio', '1:2')}`")
    ...
    with res_col2:
        ...
        st.write(f"**A+ Setup Score (0-10):** ⭐ `{sig_data.get('setup_score', 'N/A')}/10`")
