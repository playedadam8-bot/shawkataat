...
# ------------------------------------------------------------------------------
# Sidebar Configuration (Structured Flow: Day -> Step -> Exact Balance)
# ------------------------------------------------------------------------------
...
st.sidebar.markdown("---")
st.sidebar.subheader("📋 Challenge Setup Parameters")

# 1. Choose Day (Day 1, Day 2, Day 3)
selected_day_choice = st.sidebar.selectbox(
    "1️⃣ Select Trading Day", 
    ["Day 1", "Day 2", "Day 3"],
    index=0
)

# Map selected day to integer count
current_day_num = int(selected_day_choice.replace("Day ", ""))

# 2. Choose Step / Challenge Stage
selected_phase_choice = st.sidebar.selectbox(
    "2️⃣ Select Challenge Step", 
    [
...
# 3. Enter Exact Current Balance
custom_balance_input = st.sidebar.number_input(
    "3️⃣ Enter Exact Balance ($)",
    min_value=4500.0,
    max_value=15000.0,
    value=float(st.session_state.current_balance),
    step=10.0,
    help="Type your exact live account balance here."
)
...
if st.sidebar.button("Initialize / Sync $5K Bot", type="primary"):
    st.session_state.current_balance = custom_balance_input
    st.session_state.risk_per_trade = assigned_risk_pct
    st.session_state.trading_days_completed = current_day_num
    ...
    st.sidebar.success("Funding Pips Bot parameters successfully updated & synced!")
...
if st.session_state.prop_active:
    ...
    st.sidebar.markdown(f"*Active Day: **Day {st.session_state.trading_days_completed}***")
    st.sidebar.markdown(f"*Step/Stage: **{st.session_state.phase}***")
...
if not st.session_state.prop_active:
    st.info("👈 Please configure your Day, Step, and Exact Balance in the sidebar, then click **Initialize / Sync $5K Bot** to start.")
    st.stop()
...
if st.session_state.current_pnl >= st.session_state.profit_target and st.session_state.trading_days_completed >= st.session_state.min_trading_days:
    ...
    st.warning(f"⚠️ **Profit target reached (${st.session_state.current_pnl:,.2f}), but you need {st.session_state.min_trading_days} minimum trading days!** Currently completed: {st.session_state.trading_days_completed} days. Place compliant trades to fulfill the day requirement.")
...
# ------------------------------------------------------------------------------
with tab1:
    st.subheader(f"Funding Pips $5K Multi-TP Signal Generator ({st.session_state.phase} - Day {st.session_state.trading_days_completed})")
...
with tab2:
    ...
    with col_day1:
        st.write(f"**Active Trading Day:** `Day {st.session_state.trading_days_completed}`")
    with col_day2:
        if st.button("➕ Advance to Next Trading Day"):
            if st.session_state.trading_days_completed < 3:
                st.session_state.trading_days_completed += 1
                st.success(f"Advanced to Day {st.session_state.trading_days_completed}!")
                st.rerun()
            else:
                st.info("You are already on Day 3 (Maximum minimum requirement reached).")
    ...
    with col_l1:
        ...
        if st.button("Save Winning Trade", type="primary", use_container_width=True):
            ...
            st.success(f"Saved! Balance updated to ${win_bal_input:,.2f}.")
            st.rerun()
    ...
    with col_l2:
        ...
        if st.button("Save Losing Trade", type="secondary", use_container_width=True):
            ...
            st.error(f"Saved! Balance updated to ${loss_bal_input:,.2f}.")
            st.rerun()
...
if st.session_state.last_signal:
    ...
    with c1:
        ...
        st.write(f"* **TP3 (Extended):** `{sig.get('tp3')}` (⏰ `{sig.get('estimated_tp3_time', 'N/A')}`)")
