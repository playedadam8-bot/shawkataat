...
def generate_prop_vision_signal(api_key: str, image_bytes: bytes, account_balance: float, risk_pct: float):
    ...
    Selected risk percentage is {risk_pct}%, meaning the exact maximum dollar risk must be calculated from this exact balance as ${exact_risk_dollar:,.2f}.
    Deeply analyze this chart screenshot. Provide:
    ...
    7. Suggested Lot Size mathematically matched to the Entry and SL distance to risk exactly ${exact_risk_dollar:,.2f} based on a balance of ${account_balance:,.2f}.
...
if st.session_state.prop_active:
    st.sidebar.markdown("---")
    st.sidebar.subheader("📈 Live Balance Monitor")
    st.sidebar.metric("AI Synchronized Balance", f"${st.session_state.current_balance:,.2f}")
    st.sidebar.markdown(f"*Current risk size per trade: **{risk_per_trade}%** (${st.session_state.current_balance * (risk_per_trade / 100.0):,.2f})*")
...
# ------------------------------------------------------------------------------
with tab1:
    st.subheader("AI Vision Chart Analyzer (Fully Synced with Your Balance)")
    
    # Always show and allow editing of the exact balance feeding the AI
    ...
    with col_bal_1:
        st.session_state.current_balance = st.number_input(
            ...
            step=10.0,
            help="The AI uses this exact balance in real-time to calculate your risk and lot sizes."
        )
    with col_bal_2:
        st.metric("Active Risk Capital", f"${st.session_state.current_balance:,.2f}", f"{risk_per_trade}% Risk")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Generate AI Signal with Exact Balance Sizing", type="primary"):
            with st.spinner("AI is analyzing chart using your exact current balance..."):
                try:
...
with tab2:
    ...
    st.markdown("When your trade finishes (win or loss), type your **exact account balance** below so the AI stays fully informed and synchronized for your next screenshot upload.")

    col_log1, col_log2 = st.columns(2)
    
    estimated_gain = st.session_state.current_balance * (risk_per_trade / 100.0) * 2.0
    estimated_loss = st.session_state.current_balance * (risk_per_trade / 100.0)
...
# ------------------------------------------------------------------------------
# Display Active Signal Output Area
# ------------------------------------------------------------------------------
